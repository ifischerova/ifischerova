...
(🇬🇧 / 🇨🇿) one-pager that's tested and shipped the same way real products are: with a test
suite, CI, and automated audits.
...
[![Built with Eleventy](https://img.shields.io/badge/built%20with-Eleventy%203-1c1e29)](https://www.11ty.dev/)
[![Styled with Tailwind](https://img.shields.io/badge/styled%20with-Tailwind%20CSS-2dd4bf)](https://tailwindcss.com/)
...
- 🌍 **Instant bilingual toggle** — English ⇄ Czech with no page reload. Auto-detects the
...
- ✅ **Tested like a product** — a Playwright suite guards the content, the language switch,
  the contact links, and even that *no Twitter ever sneaks back in*.
- 🤖 **CI + scheduled health checks** — every push is tested, and the live site is re-tested
  daily (with an email if anything looks off).
- 🚦 **Lighthouse audits** — performance / accessibility / best-practices / SEO, on demand and weekly.
...
- **`Tests`** workflow — runs on every push/PR (build + test locally) and **daily at 06:00 UTC**
  against the live site. The live run **emails its result** so everything can be monitored
...
Made with curiosity, a lot of test cases, and the occasional intentionally-broken thing. 💚
